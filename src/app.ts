...
prClient.getPullRequests().then(prs => prClient.applyLatestBuilds(prs)).then(prBuilds => {
    ...
    prBuilds.forEach(prBuild => {
        ...
        const age = Math.floor((Date.now() - prBuild.pr.creationDate.getTime()) / 36e5 * 10) / 10;
        ...
        if (prBuild.pr.isDraft) {
            tableCellTitle.classList.add("u-danger");
        }
        tableCellTitle.innerText = `${prBuild.pr.isDraft ? "[DRAFT]" : ""} ${prBuild.pr.title}`;
